...
function z4h() {
  ...
  case $ARGC-$1 in
    ...
    <2->-ssh)
      ...
      local copy_files=(.zshrc .p10k.zsh)

      # Tar, compress and base64-encode the subset of $copy_files that actually exist.
      local dump
      dump=$(cd -- ${ZDOTDIR:-~} && tar -czhT <(print -rl -- $^copy_files(N)) | base64) || return
      ...
      local cmd='
        ...
        Z4H_DIR="${XDG_CACHE_HOME:-$HOME/.cache}"/zsh4humans.ssh
        export ZDOTDIR="$Z4H_DIR"/dotfiles
        export HISTFILE="$Z4H_DIR"/.zsh_history
        export Z4H_DIR="$Z4H_DIR"/cache
        export Z4H_CHECK_LOGIN_SHELL=0
        export Z4H_SSH=1
    ...
    1-reset)
      print -Pru2 -- "%F{3}z4h%f: deleting %B\$Z4H_DIR%b (%U${Z4H_DIR//\%/%%}%u)."
    ...
    *)
      ...
      print -Pru$fd -- "usage: %F{2}z4h%f %Binit%b"
...

  {
    if [[ ! -e $Z4H_DIR/.last-update-ts ]]; then
      ...
      if zstat -A last_update_ts +mtime -- $Z4H_DIR/.last-update-ts 2>/dev/null &&
        ...
        (( update )) || print -Pru2 -- "%F{3}z4h%f: type %F{2}z4h%f %Bupdate%b to update"
        print -n >$Z4H_DIR/.last-update-ts || return
      fi
    fi
    ...
    if [[ ! -e $Z4H_DIR/junegunn/fzf/bin/fzf || $update == 1 ]]; then
      ...
      local BASH_SOURCE=($Z4H_DIR/junegunn/fzf/install) err
    ...
    if (( $? )); then
      ...
      print -Pru2 -- "If this problem persists, type \`%F{2}z4h%f %Breset%b\`."
    fi
  }

  # Check whether the current shell is the login shell. If not, offer to change login shell.
  (( Z4H_CHECK_LOGIN_SHELL && UID && EUID )) || return 0
  [[ -n $SHELL && $SHELL != $_z4h_zsh && $_z4h_zsh == /* && ${SHELL:A} != ${_z4h_zsh:A} &&
     -x ${_z4h_zsh:A} && ! -e $Z4H_DIR/.no-check-login-shell && $+commands[chsh] == 1 &&
  ...
  (
    ...
    while true; do
      ...
      if [[ $REPLY != y ]]; then
        print -rn >$Z4H_DIR/.no-check-login-shell || return
        >>$TTY print -Pr -- "Won't ask again unless %U\$Z4H_DIR/.no-check-login-shell%u is deleted."
        return 1
      fi
      query="Try again?"
      if [[ "$(</etc/shells)" != *((#s)|$'\n')($_z4h_zsh|${_z4h_zsh:A})((#e)|$'\n')* ]]; then
...
# Enable zmv command (type `zmv` for usage).
autoload -Uz zmv

# Usage: md /some/dir
#
# The same as: mkdir -p /some/dir && cd /some/dir
